// import React from 'react';
//  import { BrowserRouter as Router,Routes,Route,Link } from 'react-router-dom';  

// import Registration from './Components/Registration';
// import Home from './Components/Home';
// import AllCourses from './Components/AllCourses';
// import PhotoGallery from './Components/PhotoGallery';
// import Mern from './Components/Mern';
// import ContactUs from './Components/ContactUs';
// import './App.css';
// import Java from './Components/Java';
// import Python from './Components/Python';



// function App() {
//   return (
    
//     <Router>

//       <div>
      
//         <h1>LIVE VIRTUAL CLASS</h1>

//         <nav class="navbar navbar-expand-sm bg-dark navbar-dark fixed-top">
//           <a class="navbar-brand" href="#">Logo</a>
//           <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarContant">
//                 <span class="navbar-toggler-icon"></span>

//             </button>
//             <div class="collapse navbar-collapse" id="navbarContant">
//           <ul class="navbar-nav">
//             <li class="nav-item">
//               <Link class="nav-link" to="./Home">Home</Link>
//             </li>
//             <li class="nav-item">
//               <Link class="nav-link" to="./AllCourses ">AllCourses</Link>
//             </li>

//             <li class="nav-item">
//             <Link class="nav-link" to="./Registration ">Registration <span class="sr-only"></span></Link>
//             </li>
//             <li class="nav-item">
//               <Link class="nav-link" to="./PhotoGallery">PhotoGallery</Link>
//             </li>
//             <li class="nav-item">
//               <Link class="nav-link" to="./ContactUs">ContactUs</Link>
//             </li>
//             <li class="nav-item">
//               <Link class="nav-link" to="./CreateCalendar">CreateCalendar</Link>
//             </li>

//           </ul>
//           </div>
//         </nav>
        

 

//         <Routes>

//           <Route path="/Home" element={<Home />} />
//           <Route path="/all-courses" element={<AllCourses />} />
//           <Route path="/Registration" element={<Registration />} />
//           <Route path="/PhotoGallery" element={<PhotoGallery />} />
//           <Route path="/ContactUs" element={<ContactUs />} />
//           <Route path="/Mern" element={<Mern />} />
//           <Route path="/Java" element={<Java />} />
//           <Route path="/Python" element={<Python />} />
          

//         </Routes><br></br><br></br>
//         <hr></hr>
//       </div>
//       <footer id="footer" class="footer-1">
//         <div class="main-footer widgets-dark typo-light">
//           <div class="container">
//             <div class="row">

//               <div class="col-xs-12 col-sm-6 col-md-3">
//                 <div class="widget subscribe no-box">
//                   <h5 class="widget-title">G-Mart<span></span></h5>
//                   <p> G-Mart is a Supermarket helps by allowing you to avoid the chore of grocery shopping and instead enjoy a simple, enjoyable way of browsing and purchasing groceries. From the comfort of your own home or workplace, you may discover new things and shop for all of your food and grocery needs at India’s best grocery store. </p>
//                 </div>
//               </div>

//               <div class="col-xs-12 col-sm-6 col-md-3">
//                 <div class="widget no-box">
//                   <h5 class="widget-title">Quick Links<span></span></h5>
//                   <ul class="thumbnail-widget">
//                     <li>
//                       <div class="thumb-content"><a href="#.">Home</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Shop by categaories</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Near By Stores</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Image Gallery</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">About Us</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Contact Us</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Help</a></div>
//                     </li>
//                   </ul>
//                 </div>
//               </div>

//               <div class="col-xs-12 col-sm-6 col-md-3">
//                 <div class="widget no-box">
//                   <h5 class="widget-title">CATEGORIES<span></span></h5>
//                   <ul class="thumbnail-widget">
//                     <li>
//                       <div class="thumb-content"><a href="#.">Grocery & Staples</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Personal Care</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Kitchen Items</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Dairy Products</a></div>
//                     </li>
//                     <li>
//                       <div class="thumb-content"><a href="#.">Stationery</a></div>
//                     </li>

//                   </ul>
//                   <a class="btn" href="https://bit.ly/3m9avif" target="_blank">Subscribe Now</a>
//                 </div>
//               </div>

//               <div class="col-xs-12 col-sm-6 col-md-3">

//                 <div class="widget no-box">
//                   <h5 class="widget-title">Contact Us<span></span></h5>

//                   <p><a href="mailto:info@domain.com" title="glorythemes">Gmart@gmai.com</a></p>
//                   <ul class="social-footer2">
//                     <ul class=""><a title="youtube" target="_blank" href="https://bit.ly/3m9avif"><img alt="youtube" width="30" height="30" src="data:image/png;base64," /></a></ul>
//                     <ul class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64," /></a></ul>
//                     <ul class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64," /></a></ul>
//                     <ul class=""><a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64," /></a></ul>
//                   </ul>

//                 </div>
//               </div>

//             </div>
//           </div>
//         </div>

//         <div class="footer-copyright">
//           <div class="container">
//             <div class="row">
//               <div class="col-md-12 text-center">
//                 <p>Copyright G-Mart © 2023. All rights reserved.</p>
//               </div>
//             </div>
//           </div>
//         </div>
       
//       </footer>
      
//     </Router>


//   );
// }


// export default App;

/*import React, { useState } from 'react';
function CountApp() {
    // Declare a new state variable, which we'll call "count"
    const [count, setCount] = useState(0);

    return (
      <div>
        <p>You clicked {count} times</p>
        <button onClick={() => setCount(count + 1)}>
          Click me
        </button>
      </div>
    );
  }
}
  export default CountApp;
/*
import React, { useState, useEffect } from 'react';
function CounterExample() {
  const [count, setCount] = useState(0);
  // Similar to componentDidMount and componentDidUpdate:
  useEffect(() => {
    // Update the document title using the browser API
    document.title = `You clicked ${count} times`;
  });
  return (
    <div>
      <p>You clicked {count} times</p>
      <button onClick={() => setCount(count + 1)}>
        Click me
      </button>
    </div>
  );
}
export default CounterExample;

*/
/*
import logo from './logo.svg';
import './App.css';
import Home from './Components/Home';


function App() {
  return (
    <div className="App">
      <header className="App-header">
        <Home />
      </header>
    </div>
  );
}
export default App;
*/
/*
import logo from './logo.svg';
import './App.css';
import AxiosExample from './Components/AxiosExample';
function App() {
  return (

    <div className="App">
      <header className="App-header">
        <AxiosExample/>

      </header>
    </div>
  );
}
export default App; */

/*import logo from './logo.svg';
import './App.css';

function App() {
 return (
   <div className="App">
     <header className="App-header">
      Welcome to the LVC
     </header>
   </div>
 );
}
export default App; */
/*
import logo from './logo.svg';
import './App.css';




function App() {
  return (
    <div className="App">
      <header className="App-header">
     <h1> Welocme TO lVC!!</h1>
      </header>
    </div>
  );
}

export default App;
*/
//const Home = () => <h1>Welcome to the Home Page!</h1>;
//const AllCourses = () => <h1>All Courses</h1>;
//const Registration = () => <h1>Registration</h1>; 
//const PhotoGallery = () => <h1>Photo Gallery</h1>;
//const Contact = () => <h1>Contact</h1>;

//class App extends React.Component {
// render() {














// src/App.js
import React from 'react';
import { BrowserRouter as Router, Route, Routes, Navigate } from 'react-router-dom';
import LoginPage from './Components/LoginPage';
import RegisterPage from './Components/RegisterPage';
import ForgotPasswordPage from './Components/ForgotPasswordPage';
import ResetPasswordPage from './Components/ResetPasswordPage';
import './App.css';
import ProfilePage from './Components/ProfilePage';

const PrivateRoute = ({ element: Component }) => {
  const isAuthenticated = localStorage.getItem('loggedInUser');
  return isAuthenticated ? <Component /> : <Navigate to="/" />;
};

function App() {
  return (
    <Router>
      <Routes>
        <Route path="/SPA-001" element={<LoginPage />} />
        <Route path="/register" element={<RegisterPage />} />
        <Route path="/forgot-password" element={<ForgotPasswordPage />} />
        <Route path="/reset-password" element={<ResetPasswordPage />} />
        <Route path="/profile" element={<PrivateRoute element={ProfilePage} />} />
      </Routes>
    </Router>
  );
}

export default App;
